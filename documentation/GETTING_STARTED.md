# Getting Started with Real-Time Transcripts (RTT) API for Webex Contact Center
...
8. [Understanding Insight Types](#understanding-insight-types)
...
The Real-Time Transcripts (RTT) API is a gRPC-based streaming service that provides real-time access to **text transcriptions** from Webex Contact Center agent-customer conversations. Unlike media forking (which provides raw audio), the RTT API delivers pre-processed transcripts generated by Webex's transcription engine.
...
- **Historical:** Retrieve transcripts from the start of the conversation
...
### 2. Supervisor Monitoring & AI-Assisted Coaching
...
- Generated against a **machine account** for the customer's organization
...
**Deployment Model:**
- Each customer organization has its own machine account and tokens
- Partners deploy their application per customer
- Tokens are customer-specific and grant access only to that customer's conversations
...
- [ ] **Active WXCC Organization:** Access to a Webex Contact Center organization (customer's org)
- [ ] **Real-Time Transcripts Enabled:** The same feature that powers Agent Desktop transcripts must be enabled
- [ ] **CCAI Features Configured:** Transcription service must be active for the customer's queues
- [ ] **Admin Access:** Ability to create machine accounts and generate tokens in customer's org
- [ ] **Conversation IDs:** Access to conversation/call IDs you want to monitor
...
- [ ] **Machine Account:** Created in Control Hub for API access
- [ ] **Access Token:** Generated with required scopes (`cjp-ccai:read,cjp:organization`)
- [ ] **Token Refresh Strategy:** Plan for token lifecycle management
